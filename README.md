...
cat << 'EOF' > /etc/nginx/conf.d/default.conf
# Зона ограничения частоты запросов (для GET Flood)
limit_req_zone $binary_remote_addr zone=l7_limit:10m rate=5r/s;

# Зона ограничения количества соединений с одного IP (для WebSocket)
limit_conn_zone $binary_remote_addr zone=conn_limit_per_ip:10m;

# Динамический апгрейд для WebSocket
map $http_upgrade $connection_upgrade {
    default upgrade;
    '' close;
}
...
# Назначить namespace botnet уровень baseline (запрещает hostNetwork и privileged)
